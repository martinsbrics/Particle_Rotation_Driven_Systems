...
n_part = 4
n_virtual = 92 # options are 92, 44 , 20 , and 0
# H_ext=10.0
phi = 12
H_ext = 6.20 #earth field
#H_ext = 11.00 #almost critical
H_fiel = (H_ext, 0.0, 0)
g_c = (0.0, -2.0, 0.0)
...
if n_virtual >0:
    for i in range(n_part):
        for j in range(n_virtual):
            system.part.add(type=Ptype[j], pos=np.add(system.part[i].pos,
                            ry.dot(Points[j]))).vs_auto_relate_to(system.part[i].id)
