...
def print_mathematica(filename):
    ...
    for i in range(n_part):
        ...
        for j in range(8):
            ...
            system.part[i * 92 + n_part + j].pos[0], system.part[i * 92 + n_part + j].pos[1],
            system.part[i * 92 + n_part + j].pos[2], radius_coners))
        for j in range(8, 20):
            fout.write(", {White,Ball[{%.15f, %.15f, %.15f}, %.15f]}" % (
            system.part[i * 92 + n_part + j].pos[0], system.part[i * 92 + n_part + j].pos[1],
            system.part[i * 92 + n_part + j].pos[2], radius_midpoints))
        for j in range(20, 44):
            fout.write(", {Magenta,Ball[{%.15f, %.15f, %.15f}, %.15f]}" % (
            system.part[i * 92 + n_part + j].pos[0], system.part[i * 92 + n_part + j].pos[1],
            system.part[i * 92 + n_part + j].pos[2], radius_red))
        for j in range(44, 68):
            fout.write(", {Red,Ball[{%.15f, %.15f, %.15f}, %.15f]}" % (
            system.part[i * 92 + n_part + j].pos[0], system.part[i * 92 + n_part + j].pos[1],
            system.part[i * 92 + n_part + j].pos[2], radius_red1))
        for j in range(68, 92):
            fout.write(", {Green,Ball[{%.15f, %.15f, %.15f}, %.15f]}" % (
            system.part[i * 92 + n_part + j].pos[0], system.part[i * 92 + n_part + j].pos[1],
            system.part[i * 92 + n_part + j].pos[2], radius_red2))
...
# #kinks
# for i in range(np.int_(n_part/2)):
#     if i%2==0:
#         Porent[2*i]=1
#         Porent[2*i+1] = 1
#     else:
...
# #!no rotation case
# rpos = np.array([0, 0, 0])
# system.part.add(id=0, type=0, pos=rpos, dip=dipole, gamma=visc, gamma_rot=1, rotation=[1, 1, 1], fix=[1, 1, 1])
# rpos = Pkink
# system.part.add(id=1, type=0, pos=rpos, dip=dipole, gamma=visc, gamma_rot=1, rotation=[1, 1, 1], fix=[1, 1, 1])
#
...
system.force_cap = 1
while i < warm_n_time:
    print('{0} {1} {2} {3} {4} {5}'.format(i, np.linalg.norm(system.part[1].pos - system.part[0].pos),
                                           system.analysis.energy()["total"], system.analysis.energy()["dipolar"],
                                           system.analysis.energy()["kinetic"], system.analysis.energy()["non_bonded"]))
...
def main_thread():
    for i in range(1000):
        ...
        print('{0} {1} {2} {3} {4}'.format(i, system.analysis.energy()["dipolar"], system.analysis.energy()["kinetic"],
                                           np.linalg.norm(system.part[1].pos - system.part[0].pos),
                                           np.dot(system.part[1].dip, system.part[0].dip)))
        print('{0} {1} {2} {3} {4} {5} {6}'.format(i, np.dot(H_fiel, system.part[0].dip),
                                                   np.dot(H_fiel, system.part[1].dip),
                                                   (system.part[1].pos[2] - system.part[0].pos[2]),
                                                   (system.part[1].pos[0] - system.part[0].pos[0]),
                                                   (system.part[1].pos[1] - system.part[0].pos[1]),
                                                   np.dot(system.part[1].dip, system.part[0].dip)))
