...
//    }
  ...
  //double H1=2.5;
  vector<double> cm={-80};
  vector<double> torqx={40,50,60,70,80,90,100,110,120,130,140,150,160 };
  //vector<double> cm={-100}; 
  //vector<double> torqx={1.5};
  vector<std::pair<double, double> > sims;
  for(int i=0; i<cm.size(); i++){
    for(int j=0; j<torqx.size(); j++){
  ...
  string data;
  ofstream file;
  file.open("helix_scan_cm_-80_N_300.dat");
  if(file.is_open()){
 ...
 // sim->propagate_IMEX_BDF2(dt/10*2.5, 5e-3);
  //sim->propagate_IMEX_BDF3(dt/10*2.5, 5e-3);
      sim->propagate_VSIMEX_BDF3_DD(dt/10*2.5, 100*1e-3, 1e-7);
